      ...
      <Card className="p-5 mb-6">
        <form onSubmit={e => { e.preventDefault(); mut.mutate(); }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <div><Label>{t("quantity")}</Label>
            ...
            {selected && <div className="text-xs text-muted-foreground mt-1">{t("available")}: {selected.stok}</div>}
          </div>
            ...
            return (
              <tr key={r.id}>
                ...
                <Td className="text-right">
                  {role === "admin" ? (
                    <Button variant="ghost" className="h-8 px-2 text-destructive hover:bg-destructive/10" onClick={() => { if (confirm(t("confirm_delete_outgoing"))) del.mutate(r.id); }}>
